...
Priority order given exam scope: Kerberoast/ASREP (Sauna) ✅ → ACL abuse (Resolute) ✅ → DCSync (Administrator, HTB 2024, swapped in for Sizzle — Sizzle's guide/tooling too outdated) ← in progress → Reel (DCSync, later) → delegation/RBCD (Manager) → AD CS (Certified) if time allows.
